...
# Default config path
CONFIG_PATH = Path.home() / ".lazybricks" / "config.toml"
...
def load_lazybricks_config(config_path: Path | None = None) -> dict:
    """Load LazyBricks config from TOML file.

    Args:
        config_path: Path to config file. Defaults to ~/.lazybricks/config.toml.
...
def load_extensions(
    ...
    Args:
        ...
        config: Config dict. If None, loads from ~/.lazybricks/config.toml.

    Returns:
        List of successfully loaded extension instances.
    """
    if config is None:
        config = load_lazybricks_config()
...
def _register_builtin_extensions() -> None:
    ...
    # Billing extension
    try:
        from lazybricks.extensions.billing import BillingExtension
